...
public class CheckBox extends View {
    ...
    public CheckBox(Context context, int resId) {
        ...
        eraser2.setStrokeWidth(app.pxToDp(28));
        backgroundPaint.setStrokeWidth(app.pxToDp(2));

        textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        textPaint.setTextSize(app.pxToDp(18));/*
        textPaint.setTypeface(app.pxToDp()tTypeface("fonts/rmedium.ttf"));*/
    ...
    public void setVisibility(int visibility) {
        ...
        if (visibility == VISIBLE && drawBitmap == null) {
            try {
                drawBitmap = Bitmap.createBitmap(app.pxToDp(size), app.pxToDp(size), Bitmap.Config.ARGB_4444);
                bitmapCanvas = new Canvas(drawBitmap);
                checkBitmap = Bitmap.createBitmap(app.pxToDp(size), app.pxToDp(size), Bitmap.Config.ARGB_4444);
    ...
    public void setSize(int size) {
        this.size = size;
        if (size == 40) {
            textPaint.setTextSize(app.pxToDp(24));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawBackground || progress != 0) {
            eraser2.setStrokeWidth(app.pxToDp(size + 6));
            ...
            if (roundProgressCheckState < progressBounceDiff) {
                rad -= app.pxToDp(2) * roundProgressCheckState / progressBounceDiff;
            } else if (roundProgressCheckState < progressBounceDiff * 2) {
                rad -= app.pxToDp(2) - app.pxToDp(2) * (roundProgressCheckState - progressBounceDiff) / progressBounceDiff;
            }
            if (drawBackground) {
                paint.setColor(0x44000000);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, rad - app.pxToDp(1), paint);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, rad - app.pxToDp(1), backgroundPaint);
            }
            ...
            if (checkedText != null) {
                ...
                checkCanvas.drawText(checkedText, (getMeasuredWidth() - w) / 2, app.pxToDp(size == 40 ? 28 : 21), textPaint);
            } else {
            ...
            checkCanvas.drawCircle(getMeasuredWidth() / 2 - app.pxToDp(2.5f), getMeasuredHeight() / 2 + app.pxToDp(4), ((getMeasuredWidth() + app.pxToDp(6)) / 2) * (1 - checkProgress), eraser2);
